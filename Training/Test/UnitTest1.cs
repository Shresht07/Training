namespace Training {
   ...
   public class UnitTest1 {
      Dictionary<string, double> values = new () {
       ...
       { "3 + * 5", 0 }, { "(4 + 6", 0 }, { "2 + abc", 0 }, { "6 *", 0 }, { "3 + 2 *", 0 }, { "5 * (3 + 2))", 0 }
      };

      Dictionary<string, double> NaNCases = new () { { "sqrt -100", double.NaN }, { "log(-10+5)", double.NaN }, { "sin(sqrt-1)", double.NaN }, { "sqrt asin-1", double.NaN } };
      string[] invalidCases = { "3 + * 5", "(4 + 6", "2 + abc", "6*", "3 + 2 *", "5 * (3 + 2))" };

      [TestMethod]
      public void TestExpressionEvaluator () {
         var eval = new Evaluator ();
         foreach (var val in values.Keys) {
            var result = Math.Round (eval.Evaluate (val),10);
            var expectedRes = values[val];
            Assert.AreEqual (expectedRes, result);
         }
         foreach (var NaNCase in NaNCases.Keys) Assert.IsTrue (Double.IsNaN (eval.Evaluate (NaNCase)));
         foreach (var invalid in invalidCases) Assert.ThrowsException<EvalException> (() => eval.Evaluate (invalid));
      }
   }
}