   ...
   public Token Next (List<Token> tokens) {
      while (mN < mText.Length) {
         ...
         switch (ch) {
            ...
            case '(' or ')':
               mEval.BasePriority += ch is '(' ? 10 : -10;
               return new TPunctuation (ch);
            case '+' or '-':
               return tokens.Count is 0 || tokens[^1] is TOperator or TPunctuation { Punct: '(' } or TOpUnary ? new TOpUnary (mEval, ch) : new TOpArithmetic (mEval, ch);
